...
- Installs the Homebrew formulae listed in `lib/brew.sh`
- Installs the font casks listed in `lib/fonts.sh`
- Installs the Homebrew casks listed in `lib/casks.sh`
- Installs the Mac App Store apps listed in `lib/mas.sh`
- Installs the npm packages listed in `lib/npm.sh`
- Installs the Cursor extensions listed in `lib/cursor.sh`
- Configures the Dock order listed in `lib/dock.sh`
- Updates `.zshrc` using `lib/zshrc.sh`, enabling the Oh My Zsh `git` plugin when available, loading `nvm` in future zsh sessions, plus `autocd` and dot-directory shortcuts
- Prints the manual follow-up steps listed in `lib/postflight.sh`
